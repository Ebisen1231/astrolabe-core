...
flowchart LR
  ...
  T[常駐チューター] --> S
  S --> L
  W[週次自己改修 flagship] --> I[private Issue 発注書ドラフト]
  I --> T2[build_app_feature 学習タスク]
```
...
1週間分のフィードバックと不満点をflagshipで傾向分析し、private ledgerのIssueへ
Codex向け発注書ドラフトを作る。同じ提案を `build_app_feature` 学習タスクとして台帳へ
流し、施主が内容を確認してからCodexへ手動発注する。Actions上でCodex Pro認証や
自動PR作成は行わない。提案には概念名の羅列、URL、chat_note原文、面談内容を引用しない。
...
- **v1.5(M3.5)** — マップ以外を白地の読書面にし、サイドバー、トピック詳細、
  概念バックリンク、横断検索、KaTeX、実践課題の直接タスク化を追加する。
- **v1.6(M4)** — PWA化とWebプッシュで、Notionを捨てたことで失ったモバイル体験を補う。
...
### M3.5 — 学習体験の磨き
- 「観測は夜、学習は昼」とし、星図とナビ枠以外を白地の読書面へ移す
- サイドバー、トピック詳細、practice_taskの直接タスク化、概念バックリンク、⌘K検索、
  KaTeX、タスクfilterを追加する
- export契約はschema_version 1の後方互換フィールド追加とし、過去報告の欠落を許容する
- 完了条件: 毎朝の報告が白地のブロック構造で読め、概念を辿れ、実践課題が
  ワンクリックでタスクになる
...
| 復習間隔アルゴリズムの方式 | 簡易SM-2系。イベントgradeはFSRS互換の1〜4 | 解消済み(2026-07-20、M4) |
...
### 2026-07-20 — M3.5を挿入(M4はその後)

- 施主がM3を受け入れ、M4の前に学習体験を磨くM3.5を挿入した。
- `docs/ui-research.md` のTier 1採否マトリクスをM3.5の仕様源とする。
- practice_taskとindex逆引きはschema_version 1の後方互換フィールドとして追加し、
  過去reportと既存readerを維持する。layout.json契約は変更しない。
- 認証付き `POST /v1/tasks` は既存のcreate_task核を呼び、taskとtask_created eventの
  原子的生成、owner限定、kind allowlistを維持する。
- M4のPWA/Webプッシュ、週次自己改修、復習間隔はM3.5へ前倒ししない。

### 2026-07-20 — M4開始・復習方式=簡易スケジューラ(FSRS互換イベント)・週次自己改修は発注書ドラフトまで自動化

- 施主がM3.5を受け入れ、M4第1便を開始した。
- 復習間隔は決定的な簡易SM-2系スケジューラから開始し、quiz_resultにはFSRS互換の
  grade 1〜4を追加する。eventsから再導出でき、将来は関数差し替えでFSRSへ移行する。
- 週次Actionsはflagshipによる傾向分析、private ledger Issueの発注書ドラフト、
  build_app_feature学習タスク、Discord通知までを自動化する。
- Codex Pro認証、Codex実行、Pull Request作成は自動化せず、施主が手動発注する。
- PWA、Webプッシュ、UIでの復習表示はM4第2便へ残す。
