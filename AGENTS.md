...
施主が2026-07-19にM3第1便を受け入れ、第2便への移行を承認済み。M3は三便に分割し、
現在は **第2便(常駐チューター核)** のみを実装する。第2便に含むもの:

- Pythonのステートレスなチューターエンジンとstrictな台帳ツール
- CLI対話と127.0.0.1固定のローカルHTTP API
- タスク生成・完了、クイズ、面談、明示的フィードバックの台帳記録
- チューター帰属分と全flagship使用量の二重予算遮断
- Next.jsのローカル `/tutor` と実タスク画面

以下はM3第2便の範囲外であり、先回りして作らないこと:
...
- `pytest`がオフライン・APIキーなしで全緑。第1便と既存CLIを壊さない
- 未知語から説明、橋渡しtask、task_created event、prerequisite edgeが一続きで生まれる
- クイズの採点がquiz_result eventとconcept confidenceへ反映される
- 面談によるprofile更新とinterview eventが同一トランザクションで記録される
- チューター帰属枠または全flagship枠の超過時、LLMを呼ばずに拒否する
- `/tutor` と `/tasks` が127.0.0.1固定のローカルAPIを通して動く
- Supabaseのタスク作成・完了RPCは施主レビュー後にSQL Editorで適用する
