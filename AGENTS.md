...
施主が2026-07-18にM0完了とM1移行を承認済み。現在は
**M1(自動化とHTML報告)** を実装する。M1に含むもの:

- private の `astrolabe-ledger` リポジトリで動く日次GitHub Actions
- 決定的コードで生成する単一ファイルHTML報告とCytoscape.jsの学習マップ
- ledgerリポジトリのprefilled GitHub Issueを使う4種フィードバックとeventsへの反映
- Discord webhook通知
- 選択・既知・dismissed履歴を翌朝の一次選別へ反映

以下はM1の範囲外であり、先回りして作らないこと:
...
- `pytest` がオフライン・APIキーなしで全緑。HTML生成、フィードバック取り込み、
  プロンプト注入のテストを含む
- `astrolabe morning --dry-run` が従来どおり通り、HTMLも一時ディレクトリに生成される
- 実HTMLをブラウザで開き、星図マップと4種フィードバックリンクが機能する
- ledgerのopenな`[fb]` Issueが次のmorningでeventsへ一度だけ反映され、Issueが閉じる
- `workflow_dispatch` が最後まで通り、ledgerにDB/HTMLコミットが積まれ、Discordへ届く
- 翌朝のschedule実行が人手なしで届き、選択が一次選別入力へ反映される
