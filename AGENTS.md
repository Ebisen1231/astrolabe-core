...
## 現在のフェーズ: M4第1便 復習間隔と週次自己改修

施主が2026-07-20にM3.5を受け入れ、M4第1便を開始した。本便に含むもの:

- 決定的なSM-2系復習スケジューラとFSRS互換gradeイベント
- 朝報告・チューターへの期日到来レビュー接続
- 直近7日の傾向からprivate ledger Issueの発注書ドラフトを生成するweekly-review
- 改修提案をbuild_app_feature学習タスクとして既存create_task核へ流す
- 週次ActionsはCodexを実行せず、施主が手動発注するドラフトまでに限定する

以下はM4第1便の範囲外であり、先回りして作らないこと:

- PWA、Webプッシュ、UIでの復習表示
- 精読・下読み工程、Hacker News収集
...
- 同一eventsから同一期日を返し、4段階gradeの間隔遷移がpytestで固定される
- grade付きquiz_resultから朝報告の「今日の復習」まで多日シミュレートできる
- weekly-reviewがprivate ledger Issue、build_app_feature task、task_created eventを作る
- 提案は概念名・URL・chat_note原文を漏らさず、再実行で副作用が重複しない
- ledger workflow_dispatchが完走し、DiscordへIssueリンクが通知される
- M0〜M3.5の朝ジョブ・チューター・UI・認証経路に回帰がない
